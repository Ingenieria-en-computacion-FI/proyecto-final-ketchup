...
int mm_allocate_worst_fit(MemoryManager* mm, int pid, int size) {
    ...
    // 1. Delegamos la decisión al algoritmo Greedy: WORST FIT
    // ESTA ES LA ÚNICA LÍNEA QUE CAMBIA EN LA LÓGICA PRINCIPAL
    MemoryBlock* target_block = algo_greedy_worst_fit(mm, size);
